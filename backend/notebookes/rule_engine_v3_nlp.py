...
# NLP enrichment layer (optional — set use_nlp=False to skip for offline runs)
import sys as _sys
_sys.path.insert(0, str(Path(__file__).parent))  # ensure nlp_extractor is findable
try:
    from nlp_extractor import NLPExtractor, apply_nlp_to_rule_engine_input
    _NLP_AVAILABLE = True
except ImportError as _e:
    print(f"[NLP] Warning: could not import nlp_extractor ({_e}). NLP disabled.")
...
def evaluate_category_rules(
    ...
    for rule in rules:
        ...
        # Rules with numeric conditions
        if rule.condition_value is not None:
            if rule.condition_currency:
                # Value-based condition (e.g. > EUR 100,000)
                if contract_value_eur >= rule.condition_value * FX_TO_EUR.get(rule.condition_currency, 1.0):
                    triggered = True
                    context = f"contract value EUR {contract_value_eur:,.0f} ≥ {rule.condition_currency} {rule.condition_value:,.0f} threshold"
            elif rule.condition_unit:
                # Quantity/day-based condition (e.g. > 50 units, > 60 days)
                if quantity >= rule.condition_value:
                    triggered = True
                    context = f"quantity {quantity} {unit_of_measure} ≥ {rule.condition_value} {rule.condition_unit} threshold"
        else:
            # Unconditional rules (e.g. brand_safety, design_signoff)
            triggered = True
            context = "unconditional for this category"

        # FIX-03: fast_track is a RELAXING rule — record it so the engine
        # can apply a lower quote requirement
        if triggered:
            applicable.append({
...
class ProcurementRuleEngine:
    ...
    def process(self, request: dict) -> dict:
        ...
        return {
            ...
            "validation": {
                "completeness": "fail" if missing_fields else "pass",
                "missing_fields": missing_fields,
...
def run_batch(
    ...
    use_nlp: bool = True,
    nlp_verbose: bool = True,
) -> list[dict]:
    """
    Run the full pipeline:
      1. (Optional) Enrich requests with NLPExtractor (Gemini 2.5 Pro)
      2. Pipe each enriched request through ProcurementRuleEngine

    Set use_nlp=False for fast/offline runs without API calls.
    """
    engine = ProcurementRuleEngine(data_dir)
    raw_requests = json.loads(requests_path.read_text())
    if max_requests:
        raw_requests = raw_requests[:max_requests]

    # ── NLP enrichment ────────────────────────────────────────────────────────
    if use_nlp and _NLP_AVAILABLE:
        print("\n[NLP] Enriching requests with Gemini 2.5 Pro...")
        extractor = NLPExtractor()
        enriched_requests = extractor.enrich_batch(raw_requests, verbose=nlp_verbose)
        # Patch structured fields with NLP findings
        requests_to_process = [
            apply_nlp_to_rule_engine_input(r) for r in enriched_requests
        ]
        print(f"[NLP] Enrichment complete. Passing to rule engine...\n")
    elif use_nlp and not _NLP_AVAILABLE:
        print("[NLP] WARNING: nlp_extractor not importable — running without NLP.")
        requests_to_process = raw_requests
    else:
        print("[NLP] Skipped (use_nlp=False).")
        requests_to_process = raw_requests
    ...
    print(f"\nProcessed {len(results)}/{len(requests_to_process)} requests → {output_path}")
    if errors:
...
if __name__ == "__main__":
    ...
    # Optional flags
    skip_nlp = "--no-nlp" in args
    args = [a for a in args if not a.startswith("--")]
    max_n = int(args[0]) if args else None

    results = run_batch(
        requests_path, data_dir, output_path,
        max_requests=max_n,
        use_nlp=not skip_nlp,
    )
