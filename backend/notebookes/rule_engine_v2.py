...
class PolicyEngine:
    ...
    def process_request(self, req: dict) -> PolicyResult:
        ...
        # GAP 3: Budget impossibility (needs candidate pool for real prices) ──
        self._check_budget_impossibility(
            sr, candidates, contradictions, escalations, log)
        ...
        # GAP 6: Split-sourcing suggestion (runs after ER-006 fires) ─────────
        if any(e.rule_id == "ER-006" for e in escalations):
            self._suggest_split_sourcing(sr, candidates, escalations, log)
        ...
        # GAP 2: Lead-time feasibility (after pool is final) ──────────────────
        self._check_lead_time(sr, candidates, escalations, log)

        # ── 8. Geography rules (GAP 7 fix: per-country) ──────────────────────
    ...
    def _check_budget_impossibility(self, sr, candidates, contradictions,
                                    escalations, log):
        """
        If budget < cheapest_supplier × quantity the request is physically
        impossible to fulfil. Flag as contradiction + ER-001.
        Called after the candidate pool is built so we have real prices.
        """
        if not candidates or sr.quantity is None or sr.budget_amount is None:
            return
        min_price = min(
            (r.get("unit_price", math.inf) for r in candidates
             if r.get("unit_price")), default=None
        )
        if min_price is None or min_price == math.inf:
            return
        min_total = min_price * sr.quantity
        if min_total > sr.budget_amount:
            shortfall = min_total - sr.budget_amount
            msg = (
                f"Budget impossibility: cheapest compliant option is "
                f"{min_price:,.2f} × {sr.quantity:,.0f} = "
                f"{min_total:,.2f} {sr.currency}, which exceeds stated budget "
                f"{sr.budget_amount:,.2f} {sr.currency} "
                f"by {shortfall:,.2f} {sr.currency}"
            )
            contradictions.append(msg)
            escalations.append(Escalation(
                rule_id="ER-001",
            ...
            log(f"[GAP3/BUDGET-IMPOSSIBLE] {msg}")
    ...
    def _check_lead_time(self, sr, candidates, escalations, log):
        """
        For each supplier row, compare today + standard_lead_time vs
        required_by_date. Mark rows as expedited-only where standard won't
        make it. If even expedited is too slow, flag as infeasible.
        """
        from datetime import date, timedelta
        if not sr.required_by_date or not candidates:
            return

        try:
            deadline = date.fromisoformat(sr.required_by_date)
        except ValueError:
            log(f"[LEAD] Could not parse required_by_date: {sr.required_by_date}")
            return

        today = date.today()
        days_available = (deadline - today).days
        log(f"[LEAD] Deadline {sr.required_by_date}: {days_available} days from today")

        if days_available < 0:
            msg = (f"Required delivery date {sr.required_by_date} is already "
                   f"in the past ({abs(days_available)} days ago)")
            escalations.append(Escalation(
                rule_id="ER-001",
                trigger="missing_required_information",
                target="Requester Clarification",
            ...
            log(f"[LEAD] {msg}")
            # Mark all rows as past-deadline
            for row in candidates:
                row["_lead_infeasible"] = True
                row.setdefault("_lead_notes", []).append(
                    f"⚠ Deadline already passed ({abs(days_available)}d ago)")
            return

        infeasible_count = 0
        expedited_only_count = 0

        for row in candidates:
            std  = int(row.get("standard_lead_time") or 0)
            exp  = int(row.get("expedited_lead_time") or 0)

            if std == 0:
                continue  # no lead time data — skip

            if days_available >= std:
                continue  # standard delivery works fine

            if days_available >= exp:
                # Standard won't make it but expedited will
                row["_expedited_only"] = True
                # Swap price to expedited
                ep = row.get("expedited_unit_price")
                if ep:
                    row["_original_unit_price"] = row.get("unit_price")
                    row["unit_price"] = ep
                row.setdefault("_lead_notes", []).append(
                    f"⚡ Expedited delivery required: std={std}d > "
                    f"{days_available}d available (exp={exp}d)"
                )
                expedited_only_count += 1
                log(f"[LEAD] {row['supplier_id']} {row['supplier_name']}: "
                    f"standard {std}d too slow, expedited {exp}d viable")
            else:
                # Even expedited is too slow
                row["_lead_infeasible"] = True
                row.setdefault("_lead_notes", []).append(
                    f"✗ Cannot meet deadline: exp={exp}d > "
                    f"{days_available}d available"
                )
                infeasible_count += 1
                log(f"[LEAD] {row['supplier_id']} {row['supplier_name']}: "
                    f"even expedited {exp}d too slow — infeasible")

        if infeasible_count > 0:
            log(f"[LEAD] {infeasible_count} supplier(s) cannot meet deadline "
                f"even with expedited delivery")
        if expedited_only_count > 0:
            log(f"[LEAD] {expedited_only_count} supplier(s) require expedited "
                f"delivery (price uplift applied)")

    # ── 9. Geography rules ───────────────────────────────────────────────────

    def _apply_geography_rules(self, sr, candidates, escalations, log):
        """
        GAP 7 FIX: evaluate every delivery country independently so that
        multi-country requests get per-jurisdiction compliance notes rather
        than one aggregate note shared across all countries.
        """
        triggered: set[str] = set()

        def attach(rule, country_context: str):
            rid  = rule["rule_id"]
            text = rule.get("rule_text") or rule.get("rule", "")
            key  = f"{rid}::{country_context}"
            if key not in triggered:
                triggered.add(key)
                log(f"[GEO] {rid} [{country_context}]: {text}")
            # Attach note scoped to the specific country/region
            for row in candidates:
                row.setdefault("_geo_notes", [])
                note = f"{rid} [{country_context}]: {text}"
                if note not in row["_geo_notes"]:
                    row["_geo_notes"].append(note)

        # Country-level rules — one evaluation per delivery country
        for country in sr.delivery_countries:
            rule = self._geo_index.get(country.upper())
            if rule:
                attach(rule, country)

        # Region-level rules — check every delivery country against region sets
        for country in sr.delivery_countries:
            c_region = COUNTRY_REGION.get(country.upper(), "")
            for region_key, rule in self._geo_index.items():
                # Skip already handled country keys
                if len(region_key) <= 3:
                    continue
                covered = set(rule.get("countries", []))
                if country.upper() in covered:
                    attach(rule, f"{country}/{c_region}")
    ...
    def _calculate_total_value(self, sr, candidates, log) -> float | None:
        """
        GAP 4 FIX: Use the more conservative of stated budget vs computed
        best-price × quantity. A stated budget below the real cost doesn't
        reduce the approval tier — it raises a contradiction instead.
        """
        computed = None
        if candidates and sr.quantity:
            best_price = min(
            ...
            if best_price and best_price < math.inf:
                computed = best_price * sr.quantity
                log(f"[VALUE] Computed: {best_price} × {sr.quantity:,.0f} "
                    f"= {computed:,.2f} {sr.currency}")

        if sr.budget_amount and computed:
            if abs(sr.budget_amount - computed) / max(sr.budget_amount, 1) > 0.10:
                log(f"[VALUE] Budget {sr.budget_amount:,.2f} and computed "
                    f"{computed:,.2f} differ >10% — using MAX for tier safety")
            total = max(sr.budget_amount, computed)
            log(f"[VALUE] Using max(budget={sr.budget_amount:,.2f}, "
                f"computed={computed:,.2f}) = {total:,.2f} {sr.currency}")
            return total

        if sr.budget_amount:
            log(f"[VALUE] Stated budget only: {sr.budget_amount:,.2f} {sr.currency}")
            return sr.budget_amount

        if computed:
            log(f"[VALUE] No budget — using computed: {computed:,.2f} {sr.currency}")
            return computed
    ...
    def _approval_tier(self, total_value, currency, log):
        ...
        # Normalise: USD thresholds use min_value/max_value/quotes_required/approvers
        # EUR and CHF thresholds use min_amount/max_amount/min_supplier_quotes/managed_by
        def _norm(t):
            return {
                "threshold_id":     t.get("threshold_id"),
                "currency":         t.get("currency"),
                "min_amount":       t.get("min_amount", t.get("min_value", 0)),
                "max_amount":       t.get("max_amount", t.get("max_value", 999_999_999)),
                "min_supplier_quotes": t.get("min_supplier_quotes",
                                             t.get("quotes_required", 1)),
                "managed_by":       t.get("managed_by", t.get("approvers", ["business"])),
            }

        tiers = sorted(
            [_norm(t) for t in self._thresholds if t["currency"] == currency],
            key=lambda t: t["min_amount"],
        )
        for tier in tiers:
            lo = tier["min_amount"] or 0
            hi = tier["max_amount"] or 999_999_999
    ...
    def _check_high_value_escalation(self, tier, total_value, currency,
        ...
        if tier.get("threshold_id") in high_tier_ids:
            managed = tier.get("managed_by", ["strategic_sourcing"])
            mgr = managed[-1] if managed else "strategic_sourcing"
            escalations.append(Escalation(
                rule_id="ER-003",
                trigger="value_exceeds_threshold",
                target=str(mgr).replace("_", " ").title(),
                detail=(f"Contract value {total_value:,.2f} {currency} "
                        f"requires {tier['min_supplier_quotes']} quotes and "
                        f"approval from: "
                        f"{', '.join(str(m).replace('_',' ').title() for m in managed)}"),
            ))
            log(f"[ER-003] High-value contract — escalating to "
                f"{str(mgr).replace('_', ' ').title()}")
    ...
    def _rank_candidates(self, sr, candidates, log) -> list[SupplierCandidate]:
        ...
        # GAP 5 FIX: deduplicate by supplier_id — when qty is missing we got one
        # row per pricing tier per supplier. Keep the row with the lowest unit
        # price per supplier (i.e. the best-value tier for comparison).
        seen_suppliers: dict[str, dict] = {}
        for row in candidates:
            sid = row["supplier_id"]
            up  = row.get("unit_price", math.inf) or math.inf
            if sid not in seen_suppliers or up < (seen_suppliers[sid].get("unit_price") or math.inf):
                seen_suppliers[sid] = row
        candidates = list(seen_suppliers.values())
        log(f"[GAP5] After deduplication: {len(candidates)} unique supplier(s)")

        # Lead-time: filter out infeasible rows (can't meet deadline even expedited)
        feasible = [r for r in candidates if not r.get("_lead_infeasible")]
        if len(feasible) < len(candidates):
            infeasible_names = [r["supplier_name"] for r in candidates
                                if r.get("_lead_infeasible")]
            log(f"[LEAD] Removed {len(candidates)-len(feasible)} infeasible "
                f"supplier(s) from ranking: {infeasible_names}")
            candidates = feasible
        ...
        for rank, s in enumerate(scored, start=1):
            ...
            log(f"[RANK #{rank}] {row['supplier_id']} {row['supplier_name']} "
                ...
                f"{'⚡EXP-ONLY' if row.get('_expedited_only') else ''}")
    ...
    # GAP 6 FIX: split-sourcing suggestion ────────────────────────────────────

    def _suggest_split_sourcing(self, sr, candidates, escalations, log):
        """
        Called after ER-006 fires. Finds pairs of suppliers whose combined
        monthly capacity covers the requested quantity, and appends a
        split-award suggestion to the last ER-006 escalation.
        """
        if sr.quantity is None:
            return

        qty = sr.quantity
        # Collect unique suppliers with their capacity
        seen: dict[str, dict] = {}
        for row in candidates:
            sid = row["supplier_id"]
            if sid not in seen:
        ...
        supplier_list = list(seen.values())
        pairs = []
        for i, a in enumerate(supplier_list):
            for b in supplier_list[i+1:]:
                cap_a = a.get("capacity_per_month", 0) or 0
                cap_b = b.get("capacity_per_month", 0) or 0
                if cap_a + cap_b >= qty:
                    # Proportion: fill A to max, remainder to B
                    qty_a = min(cap_a, qty)
                    qty_b = qty - qty_a
                    cost_a = (a.get("unit_price") or 0) * qty_a
                    cost_b = (b.get("unit_price") or 0) * qty_b
                    pairs.append({
                        "a": a["supplier_name"], "sid_a": a["supplier_id"],
                        "b": b["supplier_name"], "sid_b": b["supplier_id"],
                        "qty_a": qty_a, "qty_b": qty_b,
                        "total_cost": cost_a + cost_b,
                        "combined_cap": cap_a + cap_b,
                    })

        if not pairs:
            log(f"[GAP6] No supplier pair found with combined capacity >= {qty:,.0f}")
            return

        # Pick the cheapest viable pair
        pairs.sort(key=lambda p: p["total_cost"])
        best = pairs[0]
        suggestion = (
            f"Split-award suggestion: {best['sid_a']} {best['a']} "
            f"({best['qty_a']:,.0f} units) + {best['sid_b']} {best['b']} "
            f"({best['qty_b']:,.0f} units) — combined capacity "
            f"{best['combined_cap']:,.0f} covers full qty {qty:,.0f}. "
            f"Est. total cost: {best['total_cost']:,.2f} {sr.currency}"
        )
        log(f"[GAP6] {suggestion}")

        # Append to last ER-006 escalation or create a new one
        er006 = next((e for e in reversed(escalations)
                      if e.rule_id == "ER-006"), None)
        if er006:
            er006.detail += f" | SPLIT: {suggestion}"
        else:
            escalations.append(Escalation(
                rule_id="ER-006",
                trigger="single_supplier_capacity_risk",
...
def format_result_json(result: PolicyResult) -> str:
    """Render a PolicyResult as a JSON string matching the requested schema."""
    import json
    ...
    if sr.required_by_date:
        try:
            deadline = date.fromisoformat(sr.required_by_date)
            days_until_required = (deadline - date.today()).days
        except ValueError:
            pass

    issues_detected = []
    # Map contradictions to issues
    ...
    escalations = []
    for i, e in enumerate(result.escalations, 1):
        escalations.append({
            "escalation_id": f"ESC-{i:03d}",
            "rule": e.rule_id,
            "trigger": e.trigger + ": " + e.detail,
            "escalate_to": e.target,
            "blocking": True
        })
    ...
    for s in result.supplier_shortlist:
        shortlist.append({
            ...
            "unit_price_eur": s.unit_price if s.currency == "EUR" else None,
            "total_price_eur": s.total_value if s.currency == "EUR" else None,
            "standard_lead_time_days": s.standard_lead,
            "expedited_lead_time_days": s.expedited_lead,
            "expedited_unit_price_eur": s.expedited_price if s.currency == "EUR" else None,
            "quality_score": s.quality_score,
            "risk_score": s.risk_score,
            "esg_score": s.esg_score,
            "policy_compliant": True,
            "covers_delivery_country": True,
            "recommendation_note": " | ".join(s.notes) if s.notes else ""
        })

    tier = result.approval_tier
    policy_evaluation = {
        "approval_threshold": {
            "rule_applied": tier.get("threshold_id") if tier else None,
            "quotes_required": result.required_quotes,
            "approvers": result.approvers,
        },
        "preferred_supplier": {
            "supplier": sr.preferred_supplier_name,
            "is_preferred": bool(sr.preferred_supplier_name)
        }
    }

    out = {
        "_note": "Agent output payload generated from PolicyResult.",
        "request_id": result.request_id,
        "processed_at": datetime.now(timezone.utc).strftime("%Y-%m-%dT%H:%M:%SZ"),
        "request_interpretation": {
            "category_l1": sr.category_l1,
            "category_l2": sr.category_l2,
            "quantity": sr.quantity,
            "budget_amount": sr.budget_amount,
            "currency": sr.currency,
            "delivery_countries": sr.delivery_countries,
            "required_by_date": sr.required_by_date,
            "days_until_required": days_until_required,
            "data_residency_required": sr.data_residency_constraint,
            "esg_requirement": sr.esg_requirement,
            "preferred_supplier_stated": sr.preferred_supplier_name,
            "incumbent_supplier": sr.incumbent_supplier,
            "requester_instruction": sr.request_text
        },
        "validation": {
            "completeness": "fail" if any(e.rule_id == "ER-001" for e in result.escalations) else "pass",
        ...
        "escalations": escalations,
        "recommendation": {
            "status": "cannot_proceed" if escalations else "proceed",
            "reason": "Blocking issues detected." if escalations else "Ready for award."
        },
        "audit_trail": {
            "logs": result.audit_trail
        }
    }
