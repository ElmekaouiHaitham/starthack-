...
SETUP
-----
  Provide GOOGLE_API_KEY in .env file
...
def call_extraction_api(request: dict, client: genai.Client) -> dict:
    """
    ...
    def process(self, request: dict, force_refresh: bool = False) -> NLPResult:
        ...
        for attempt in range(3):
            try:
                ...
                notes.append(f"JSON parse error: {str(e)[:120]}")
                if attempt == 2:
                ...
                if "429" in err_str or "quota" in err_str.lower() or "rate" in err_str.lower():
                    ...
                    notes.append(f"API error: {err_str[:100]}")
                    if attempt == 2:
                        return _fallback_result(req_id, request, notes)
...
def apply_nlp_to_rule_engine_input(enriched_request: dict) -> dict:
    """
    Patch the request dict so the rule engine works with NLP-extracted values.

    WHAT THIS DOES — ordered by priority:
      A. Translation  : sets _working_text to English translation when available
      B. Quantity fill: fills quantity when field is null OR when text clearly
                        contradicts the field (mismatch >10%) — uses text value
                        for pricing and records both values in _nlp_qty_source
      C. Budget fill  : fills budget_amount when field is null and NLP found a
                        value in text; also fills currency if null
      D. Preferred supplier fill: fills preferred_supplier_mentioned when null
                        and NLP found a supplier name in text
      E. Boolean flags: upgrades esg_requirement and data_residency_constraint
                        when NLP detected language the structured field missed
      F. Audit trail  : records every fill in _nlp_filled_fields dict so the
                        rule engine can surface what was auto-filled vs original

    WHAT THIS DOES NOT DO:
      - Override budget_amount when it already has a value (even if it differs
        from text) — that discrepancy is a CONTRADICTION to flag, not a silent fix
      - Override preferred_supplier_mentioned when already set to a different
        value — surface as a PREFERRED_SUPPLIER_MISMATCH contradiction instead
    """
    req = dict(enriched_request)
    nlp = req.get("nlp", {})
    ext = nlp.get("extraction", {})

    # Track every field that NLP fills for audit trail
    filled: dict[str, dict] = {}

    # ── A: Working text (translation) ────────────────────────────────────────
    if ext.get("english_translation"):
        req["_working_text"] = ext["english_translation"]
        filled["working_text"] = {
            "original_language": ext.get("detected_language"),
            "note": "Request translated to English for processing",
        }
    else:
        req["_working_text"] = req.get("request_text", "")

    # ── B: Quantity ───────────────────────────────────────────────────────────
    qty_field   = req.get("quantity")
    qty_text    = ext.get("qty_primary")
    qty_unit    = ext.get("qty_primary_unit")

    if qty_text is not None:
        if qty_field is None:
            # Field was null — fill from text, no contradiction
            req["quantity"]        = qty_text
            req["unit_of_measure"] = qty_unit or req.get("unit_of_measure")
            req["_nlp_qty_source"] = "text_fill"
            filled["quantity"] = {
                "original_field_value": None,
                "nlp_value": qty_text,
                "unit": qty_unit,
                "source": "text_fill",
                "note": "Quantity field was null; value extracted from request text.",
            }
        else:
            # Field has a value — check if text contradicts it
            try:
                diff = abs(qty_text - float(qty_field)) / max(float(qty_field), 1)
            except (TypeError, ValueError):
                diff = 0.0
            ...
            if diff > QTY_MISMATCH_THRESHOLD:
                # Use text value for pricing (it's more specific), flag the mismatch
                req["quantity"]        = qty_text
                req["unit_of_measure"] = qty_unit or req.get("unit_of_measure")
                req["_nlp_qty_source"] = "text_override"
                filled["quantity"] = {
                    "original_field_value": qty_field,
                    "nlp_value": qty_text,
                    "unit": qty_unit,
                    "diff_pct": round(diff * 100, 1),
                    "source": "text_override",
                    "note": (
                        f"Field quantity={qty_field} conflicts with text quantity={qty_text} "
                        f"({diff*100:.0f}% difference). Text value used for pricing. "
                        "Both values surfaced as QUANTITY_MISMATCH contradiction."
                    ),
                }
            else:
                req["_nlp_qty_source"] = "field_confirmed"

    # ── C: Budget ─────────────────────────────────────────────────────────────
    budget_field   = req.get("budget_amount")
    budget_text    = ext.get("budget_extracted")
    currency_text  = ext.get("budget_currency")

    if budget_field is None and budget_text is not None:
        req["budget_amount"] = budget_text
        req["_nlp_budget_source"] = "text_fill"
        # Also fill currency if it's missing
        if not req.get("currency") and currency_text:
            req["currency"] = currency_text
        filled["budget_amount"] = {
            "original_field_value": None,
            "nlp_value": budget_text,
            "currency": currency_text or req.get("currency"),
            "source": "text_fill",
            "note": "Budget field was null; value extracted from request text.",
        }
    elif budget_field is not None:
        req["_nlp_budget_source"] = "field_used"
        # If there's a currency mismatch, note it but don't override
        if currency_text and currency_text != req.get("currency"):
            filled["budget_currency_note"] = {
                "field_currency": req.get("currency"),
                "text_currency": currency_text,
                "note": "Currency in text differs from currency field. Surfaced as contradiction.",
            }

    # ── D: Preferred supplier ─────────────────────────────────────────────────
    pref_field = req.get("preferred_supplier_mentioned")
    pref_text  = ext.get("preferred_supplier_in_text")

    if pref_field is None and pref_text:
        req["preferred_supplier_mentioned"] = pref_text
        req["_nlp_pref_supplier_source"] = "text_fill"
        filled["preferred_supplier_mentioned"] = {
            "original_field_value": None,
            "nlp_value": pref_text,
            "source": "text_fill",
            "note": "Preferred supplier field was null; supplier name extracted from request text.",
        }
    elif pref_field and pref_text and pref_field.lower() != pref_text.lower():
        # Both have values but they differ — don't override, just note it
        req["_nlp_pref_supplier_source"] = "field_used_mismatch_noted"
        filled["preferred_supplier_mismatch"] = {
            "field_value": pref_field,
            "text_value": pref_text,
            "note": "Supplier in text differs from field. Field value used. Surfaced as contradiction.",
        }

    # ── E: Boolean flags ──────────────────────────────────────────────────────
    if ext.get("esg_signal_in_text") and not req.get("esg_requirement"):
        req["esg_requirement"] = True
        filled["esg_requirement"] = {
            "original_field_value": False,
            "nlp_value": True,
            "note": "NLP detected ESG/sustainability language not captured in structured field.",
        }

    if ext.get("data_residency_signal_in_text") and not req.get("data_residency_constraint"):
        req["data_residency_constraint"] = True
        filled["data_residency_constraint"] = {
            "original_field_value": False,
            "nlp_value": True,
            "note": "NLP detected data residency/sovereignty language not captured in structured field.",
        }

    # ── F: Audit trail ────────────────────────────────────────────────────────
    req["_nlp_filled_fields"] = filled
